...
function mostrarProyectos(proyectos) {
  ...
  proyectos.forEach((p) => {
    ...
    const nombreResp = p.responsable_nombre ? `Responsable: ${p.responsable_nombre}` : '';
      ...
      <div class="card-body">
        <div class="d-flex justify-content-between align-items-start">
          <div>
            ...
            <p class="card-text mb-2"><small class="text-muted">${nombreResp}</small></p>
          </div>
        </div>
  ...
  try {
    ...
    if (!id) {
      ...
      response = await fetch(API_URL, {
        ...
        body: JSON.stringify({ ...proyecto, responsable: selectResp.value })
      });
    } else {
      // PUT
...
function abrirEditar(proyecto) {
    ...
    selectResp.value = proyecto.responsable || '';
